/* action.c generated by valac 0.14.0, the Vala compiler
		...
		_tmp16__length1 = profile_names_length1;
		{
			...
			for (profile_name_it = 0; profile_name_it < _tmp16__length1; profile_name_it = profile_name_it + 1) {
				...
				profile_name = _tmp17_;
				{
					GKeyFile* _tmp18_;
					const gchar* _tmp19_;
					FmFileActionProfile* _tmp20_;
					_tmp18_ = kf;
					_tmp19_ = profile_name;
					_tmp20_ = fm_file_action_profile_new (_tmp18_, _tmp19_);
					self->profiles = g_list_prepend (self->profiles, _tmp20_);
...
GList* fm_get_actions_for_files (GList* files) {
	...
	g_hash_table_iter_init (&_tmp14_, _tmp13_);
	action_it = _tmp14_;
	_g_object_unref0 (action_obj);
	action_obj = NULL;
	while (TRUE) {
		...
		FmFileActionObject* _tmp18_;
		gboolean _tmp19_;
		_tmp16_ = g_hash_table_iter_next (&action_it, NULL, &_tmp15_);
		_g_object_unref0 (action_obj);
		...
		_tmp18_ = action_obj;
		_tmp19_ = _tmp18_->has_parent;
		if (_tmp19_ == FALSE) {
			FmFileActionObject* _tmp20_;
			GList* _tmp21_;
			...
			_tmp20_ = action_obj;
			_tmp21_ = files;
			_tmp22_ = fm_file_action_item_new_for_action_object (_tmp20_, _tmp21_);
			item = _tmp22_;
			_tmp23_ = item;
			if (_tmp23_ != NULL) {
				FmFileActionItem* _tmp24_;
				FmFileActionItem* _tmp25_;
				_tmp24_ = item;
				_tmp25_ = _fm_file_action_item_ref0 (_tmp24_);
				items = g_list_append (items, _tmp25_);
			}
			_fm_file_action_item_unref0 (item);
		}
	}
	_tmp26_ = fm_all_actions;
	...
	while (TRUE) {
		...
		FmFileActionType _tmp33_;
		_tmp29_ = g_hash_table_iter_next (&action_it, NULL, &_tmp28_);
		_g_object_unref0 (action_obj);
		_tmp30_ = _g_object_ref0 (_tmp28_);
		action_obj = _tmp30_;
		if (!_tmp29_) {
			break;
		}
		_tmp31_ = action_obj;
		_tmp31_->has_parent = FALSE;
		_tmp32_ = action_obj;
		_tmp33_ = _tmp32_->type;
		if (_tmp33_ == FM_FILE_ACTION_TYPE_MENU) {
			FmFileActionObject* _tmp34_;
			FmFileActionMenu* _tmp35_;
			FmFileActionMenu* menu;
			FmFileActionMenu* _tmp36_;
			_tmp34_ = action_obj;
			_tmp35_ = _g_object_ref0 (FM_FILE_ACTION_MENU (_tmp34_));
			menu = _tmp35_;
			_tmp36_ = menu;
			__g_list_free__g_object_unref0_0 (_tmp36_->cached_children);
			_tmp36_->cached_children = NULL;
			_g_object_unref0 (menu);
		}
	}
	result = items;
