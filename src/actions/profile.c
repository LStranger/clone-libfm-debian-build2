/* profile.c generated by valac 0.14.0, the Vala compiler
	...
	if (_tmp8_ == FM_FILE_ACTION_EXEC_MODE_DISPLAY_OUTPUT) {
		{
			...
			gboolean _tmp14_;
			_tmp9_ = exec_cmd;
			_tmp12_ = g_spawn_command_line_sync (_tmp9_, &_tmp10_, NULL, &_tmp11_, &_inner_error_);
			_g_free0 (_vala_output);
			_vala_output = _tmp10_;
			exit_status = _tmp11_;
			_tmp13_ = _tmp12_;
	...
	} else {
		{
			...
			gboolean _tmp18_;
			_tmp16_ = exec_cmd;
			_tmp17_ = g_spawn_command_line_async (_tmp16_, &_inner_error_);
			_tmp18_ = _tmp17_;
			if (_inner_error_ != NULL) {
				if (_inner_error_->domain == G_SPAWN_ERROR) {
